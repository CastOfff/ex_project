...
class LargeDiscountsPage extends StatefulWidget {
  ...
  const LargeDiscountsPage({super.key, required this.happyDeal});
...
class _LargeDiscountsPageState extends State<LargeDiscountsPage> with SingleTickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    // final HappyDeal happyDeal = ModalRoute.of(context)!.settings.arguments as HappyDeal;
    return BlocProvider(
      create: (context) => HappyDealsBloc()..add(HappyDealsFetchEvent(happyDeal: widget.happyDeal)),
      child: Scaffold(
        body: Stack(
          children: [
            Container(
            ...
            BlocBuilder<HappyDealsBloc, HappyDealsState>(
              builder: (context, state) {
                if (state is HappyDealsLoading) {
                  return const Center(child: CircularProgressIndicator());
                }
                if (state is HappyDealsDataLoaded){
                  return ///Than page
                    Stack(
                      children: [
                        Column(
                              ...
                              child: Align(
                                ...
                                child: TweenAnimationBuilder<double>(
                                  ...
                                  child: Image(
                                      height: 200,
                                      width: 260,
                                      image: AssetImage(state.happyDeal.image ?? 'assets/images/pngegg.png' ),
                                      fit: BoxFit.cover),
                                ),
                              ),
                            ),
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    VerifyCommonButton(
                                      onPressed: () {
                                        Navigator.pushNamed(context, RouterName.happyDealReservationPage, arguments: state.happyDeal);
                                      },
                                      title: 'GET IT NOW',
                                    ),
                                    const SizedBox(height: 20,),
                ...
                if (state is HappyDealsDataError) {
